...
NeuralNetwork::NeuralNetwork(std::vector<std::unique_ptr<Layer>> layers): layers(std::move(layers)) {}
...
NeuralNetwork& NeuralNetwork::backward(VectorXd &input, VectorXd &target, double learning_rate) {
  VectorXd output_delta = layers.back()->get_deriv_activated_values(target);
